...
public class FileService {
	...
	public void createExcelPostReport(HSSFWorkbook workbook){
	    ...
	    for(Post post : allPosts) {
	    	List<PostViewed> postsViewed = allPostViewed.stream().filter(p -> (post.getId() == p.getPost().getId())).collect(Collectors.toList());
		    ...
		    for(Blacklist bl : post.getBlacklist()) {
		    	if(bl.getCommentId() == 0 && bl.getBlacklistedUntil().isAfter(LocalDate.of(loc.getYear(), loc.getMonthValue(), loc.getDayOfMonth()))) {
		    		isBanned = true;
		    		break;
		    	}
		    }
		    ...
		    postExcel.add(new PostExcel(idPost, title, (isFree ? "Y" : "N"), totalViews, uniqueViews, (isBanned ? "Y" : "N"), avgRating, numberOfPurchases));
	    }
	    ...
	    String[] footer = {"COUNT(B3:B" + (postExcel.size() + 2) + ")",
	    		"",
	    		"",
	    		"SUM(E3:E" + (postExcel.size() + 2) + ")",
	    		"SUM(F3:F" + (postExcel.size() + 2) + ")",
	    		"COUNTIF(G3:G" + (postExcel.size() + 2) + ", \"Y\")",
	    		"",
	    		"SUM(I3:I" + (postExcel.size() + 2) + ")"};
	...
	public void createExcelReaderReport(HSSFWorkbook workbook){
	    ...
	    List<Long> usersWhoCommented = allComments.stream().map(comment -> comment.getCreatedBy()).distinct().collect(Collectors.toList());
	    
	    /* add all users who have the role READER or has at least 1 comment with DISTINCT*/
	    List<Users> allUsers = userRepository.findByRolesOrIdIn(roleReader, usersWhoCommented).stream().distinct().collect(Collectors.toList());
	    ...
	    List<Blacklist> allBlacklists = blacklistRepository.findByCommentIdInAndBlacklistedUntilAfter(idComments, LocalDate.now());
	    
	    List<ReaderExcel> readerExcel = new ArrayList<ReaderExcel>();
	    for(Users user : allUsers) {
	    	List<Comment> comments = allComments.stream().filter(c -> (user.getId() == c.getCreatedBy())).collect(Collectors.toList());
		    
	    	Long totalComments = Long.valueOf(comments.size());
	    	Long publishedComments = Long.valueOf(comments.stream().filter(c -> c.isVisible()).collect(Collectors.toList()).size());
		    
		    List<Comment> commentsNotVisible = comments.stream().filter(p -> !p.isVisible()).collect(Collectors.toList());
		    ...
		    for(Comment c : commentsNotVisible) {
		    	List<Blacklist> postBlacklist = allBlacklists.stream()
		    			.filter(b -> (b.getCommentId() == c.getId()))
		    ...
		    readerExcel.add(new ReaderExcel(
		    		user.getUsername(),
		    		totalComments,
		    		publishedComments,
		    		commentsToCheck,
	    ...
	    String[] footer = {"Total",
	    		"SUM(C3:C" + (readerExcel.size() + 2) + ")",
	    		"SUM(D3:D" + (readerExcel.size() + 2) + ")",
	    		"SUM(E3:E" + (readerExcel.size() + 2) + ")",
	    		"SUM(F3:F" + (readerExcel.size() + 2) + ")",
	    		"SUM(G3:G" + (readerExcel.size() + 2) + ")",
	...
	public void createExcelAuthorReport(HSSFWorkbook workbook){
	    ...
	    List<Long> usersWhoPosted = allPosts.stream().map(post -> post.getCreatedBy()).distinct().collect(Collectors.toList());
	    
	    /* add all users who have the  EDITOR or has created at least 1 post with DISTINCT*/
	    List<Users> allUsers = userRepository.findByRolesOrIdIn(roleEditor, usersWhoPosted).stream().distinct().collect(Collectors.toList());
	    ...
	    for(Users user : allUsers) {
	    	List<Post> posts = allPosts.stream().filter(p -> (user.getId() == p.getCreatedBy())).collect(Collectors.toList());
		    
	    	Long totalPosts = Long.valueOf(posts.size());
	    	Long publishedPosts = Long.valueOf(posts.stream().filter(p -> (p.isVisible() && p.isApproved())).collect(Collectors.toList()).size());
	    	Long postsToCheck = Long.valueOf(posts.stream().filter(p -> !p.isVisible() && p.isApproved() ).collect(Collectors.toList()).size());
		    
		    List<Post> postToVerify = posts.stream().filter(p -> (!p.isVisible() && !p.isApproved())).collect(Collectors.toList());
		    ...
		    authorExcel.add(new AuthorExcel(user.getUsername(), totalPosts, publishedPosts, postsToCheck, totalBannedPosts));
	    }
	    ...
	    String[] footer = {"Total",
	    		"SUM(C3:C" + (authorExcel.size() + 2) + ")",
	    		"SUM(D3:D" + (authorExcel.size() + 2) + ")",
	    		"SUM(E3:E" + (authorExcel.size() + 2) + ")",
	...
	public InputStream createPdfFromPost(Post p) throws Exception {   
		...
		manipulatePdf(in, out, document, FONT_AUTHOR);
    ...
    public static PdfPCell createTextCellWith2Paragraphs(Paragraph p1, Paragraph p2, int alignment) throws DocumentException, IOException {
        PdfPCell cell = new PdfPCell();
        cell.addElement(p1);
        cell.addElement(p2);
        cell.setVerticalAlignment(alignment);
    ...
    public void manipulatePdf(InputStream in, OutputStream out, Document document, Font font) throws IOException, DocumentException {
        ...
        for (int i = 0; i < n;) {
            ...
            ColumnText.showTextAligned(pagecontent, Element.ALIGN_RIGHT,
                    new Phrase(String.format("page %s of %s", i, n), font),
                    document.right(),
