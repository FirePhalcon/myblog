...
import it.course.myblog.payload.request.PostViewedDateRequest;
...
public class PostViewedController {
	...
	public ResponseEntity<ApiResponseCustom> countVisitedPost(@RequestBody PostViewedDateRequest postViewedRequest, HttpServletRequest request){
		
		
		List<PostViewed> pvs = postViewedRepository.findByViewedStartBetween(postViewedRequest.getViewedStart(), postViewedRequest.getViewedEnd());
		...
		return new ResponseEntity<ApiResponseCustom>(new ApiResponseCustom( Instant.now(), 200, null, "Posts viewed from "+postViewedRequest.getViewedStart()+" to "+postViewedRequest.getViewedEnd()+ ": "+pvs.size()+". Unique: "+pvrs.size(), request.getRequestURI()), HttpStatus.OK);
				
	}
	
	@GetMapping("/count-visits-by-posts")
	@PreAuthorize("hasRole('ADMIN')")
	public ResponseEntity<ApiResponseCustom> countVisitsByPost(@RequestBody PostViewedDateRequest postViewedRequest, HttpServletRequest request){
		...
		List<PostViewed> pvs = postViewedRepository.findByViewedStartBetweenAndPost(postViewedRequest.getViewedStart(), postViewedRequest.getViewedEnd(), p.get());
		
		List<PostViewed> pvsFilteredByIp = StreamEx.of(pvs).distinct(PostViewed::getIp).toList();
		
		List<PostViewedResponse> pvrs = pvsFilteredByIp.stream().map(PostViewedResponse::create).collect(Collectors.toList());	
		
		return new ResponseEntity<ApiResponseCustom>(new ApiResponseCustom( Instant.now(), 200, null, "Post "+p.get().getTitle()+" viewed from "+postViewedRequest.getViewedStart()+" to "+postViewedRequest.getViewedEnd()+ ": "+pvs.size()+". Unique: "+pvrs.size(), request.getRequestURI()), HttpStatus.OK);
					
	}
	
	@PutMapping("/set-post-viewer-end/{id}")
	public ResponseEntity<ApiResponseCustom> closeVisualizationPost (@PathVariable Long id, HttpServletRequest request){
